...
def test_align_camera_extrinsics(num_tests=10000):
    ...
    for test_idx in range(num_tests):
        ...
        R_tgt, T_tgt = apply_transformation(cameras_src, R_transform, T_transform, scale_transform)
        ...
        R_verify, T_verify = apply_transformation(cameras_src, align_t_R, align_t_T, align_t_s)
...
def umeyama_alignment_torch(x, y, with_scale=True):
    """
    """
    ...
    # Variance
    sigma_x = torch.sum((x - mean_x).norm(dim=0) ** 2) / n

    # Covariance matrix
    cov_xy = (y - mean_y) @ (x - mean_x).t() / n

    # SVD
    u, s, v = torch.svd(cov_xy)

    # Correction matrix S
    s_correction = torch.eye(m, device=x.device, dtype=x.dtype)
    if torch.det(u) * torch.det(v.t()) < 0:
        s_correction[-1, -1] = -1

    # Rotation
    r = u @ s_correction @ v.t()

    # Scale and translation
    c = (s * s_correction.diag()).sum() / sigma_x if with_scale else 1.0
    t = mean_y - c * (r @ mean_x)

    return r, t.squeeze(), c
