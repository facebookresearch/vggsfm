...
def conduct_pnp(points3D, points2D, intrinsics, max_ransac_iters=1024, max_error=8, lo_num=50, f_trials=51):
    ...
    points3D_ransac = points3D[:, ransac_idx].view(B * max_ransac_iters, point_per_sample, 3)
    points2D_ransac = points2D[:, ransac_idx].view(B * max_ransac_iters, point_per_sample, 2)
    intrinsics_ransac = intrinsics[:, None].expand(-1, max_ransac_iters, -1, -1).reshape(B * max_ransac_iters, 3, 3)
    pred_world_to_cam = solve_pnp_dlt(points3D_ransac, points2D_ransac, intrinsics_ransac)

    pred_world_to_cam_4x4 = kornia.eye_like(4, pred_world_to_cam)
    pred_world_to_cam_4x4[:, :3, :] = pred_world_to_cam

    points3D_expand = points3D[:, None].expand(-1, max_ransac_iters, -1, -1).reshape(B * max_ransac_iters, P, 3)
    points2D_expand = points2D[:, None].expand(-1, max_ransac_iters, -1, -1).reshape(B * max_ransac_iters, P, 2)
    cam_points = kornia.geometry.transform_points(pred_world_to_cam_4x4, points3D_expand)

    img_points = kornia.geometry.project_points(cam_points, intrinsics_ransac[:, None])
    ...
    points2D_normalized = (points2D - principal_point[:, None]) / focal_length[:, None]

    # LORANSAC refinement
    transform_lo = local_refinement(
        efficient_pnp, points3D, points2D_normalized, inlier_mask, sorted_indices, lo_num=lo_num, skip_resize=True
    )
    ...
    # all
    points3D_expand = points3D[:, None].expand(-1, all_pmat_num, -1, -1).reshape(B * all_pmat_num, P, 3)
    points2D_expand = points2D[:, None].expand(-1, all_pmat_num, -1, -1).reshape(B * all_pmat_num, P, 2)
    intrinsics_all = intrinsics[:, None].expand(-1, all_pmat_num, -1, -1).reshape(B * all_pmat_num, 3, 3)

    cam_points = kornia.geometry.transform_points(all_pmat.reshape(B * all_pmat_num, 4, 4), points3D_expand)
    img_points = kornia.geometry.project_points(cam_points, intrinsics_all[:, None])
    ...
    residuals_all = residuals_all.reshape(oriB, f_trials, all_pmat_num, P).reshape(oriB, f_trials * all_pmat_num, P)
    ...
    all_pmat = all_pmat.reshape(B, f_trials, all_pmat_num, 4, 4).reshape(B, f_trials * all_pmat_num, 4, 4)
    all_intri = intrinsics_all.reshape(B, f_trials, all_pmat_num, 3, 3).reshape(B, f_trials * all_pmat_num, 3, 3)
...
def generate_focal_factors(num_focal_length_samples=10, max_focal_length_ratio=5, min_focal_length_ratio=0.2):
    ...
    for i in range(num_focal_length_samples):
        focal_length_factors.append(min_focal_length_ratio + fscale * focal * focal)
        focal += fstep
